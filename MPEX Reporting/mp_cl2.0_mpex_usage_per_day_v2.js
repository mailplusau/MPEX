...
define(['N/email', 'N/runtime', 'N/search', 'N/record', 'N/http', 'N/log', 'N/error', 'N/url', 'N/format', 'N/currentRecord'],
    function(email, runtime, search, record, http, log, error, url, format, currentRecord) {
        ...
        function loadDebtRecord(date_from, date_to, zee_id) {
            // MPEX - Usage Report - Last year to date (per day)
            var mpexUsageResults = search.load({
                type: 'customrecord_customer_product_stock',
                id: 'customsearch_prod_stock_usage_report_6_4'
            });
        ...
        function loadDatatable(debt_rows) {
            ...
            if (!isNullorEmpty(debt_rows)) {
                debt_rows.forEach(function(debt_row, index) {
                    ...
                    var detailedInvoiceURLMonth = '<a href="https://1048144.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1271&deploy=1&zee=' + zee + '&start_date=' + startDate + '&last_date=' + lastDate + '" target=_blank>VIEW (per week)</a> <a href="https://1048144.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1271&deploy=1&zee=' + zee + '&start_date=' + startDate + '&last_date=' + lastDate + '" target=_blank>VIEW (per day)</a>';



                    debtDataSet.push([detailedInvoiceURLMonth, month, debt_row.mpexUsage, debt_row.customerCount, debt_row.zeeCount]);

                    csvSet.push([month, debt_row.mpexUsage, debt_row.customerCount, debt_row.zeeCount]);
