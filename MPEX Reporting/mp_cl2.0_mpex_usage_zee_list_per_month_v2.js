    ...
    function(email, runtime, search, record, http, log, error, url, format, currentRecord) {
        ...
        function loadDebtRecord(date_from, date_to, zee_id, source_id) {
            ...
            mpexUsageResults.run().each(function(mpexUsageSet) {
                ...
                var zeeNameText = mpexUsageSet.getText({
                    name: 'custrecord_cust_prod_stock_zee',
                    summary: 'GROUP'
                });

                var zeeName = mpexUsageSet.getValue({
                    name: 'custrecord_cust_prod_stock_zee',
                    summary: 'GROUP'
                });
        ...
        function loadDatatable(debt_rows) {
            ...
            if (!isNullorEmpty(debt_rows)) {
                debt_rows.forEach(function(debt_row, index) {
                    ...
                    var detailedInvoiceURLMonth = '<a href="https://1048144.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1271&deploy=1&zee=' + debt_row.zeeName + '&start_date=' + startDate + '&last_date=' + lastDate + '" target=_blank>VIEW (per week)</a> | <a href="https://1048144.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1273&deploy=1&zee=' + debt_row.zeeName + '&start_date=' + startDate + '&last_date=' + lastDate + '" target=_blank>VIEW (per day)</a> | <a href="https://1048144.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1276&deploy=1&zee=' + debt_row.zeeName + '&start_date=' + startDate + '&last_date=' + lastDate + '" target=_blank>VIEW (Product Types)</a> | <a href="https://1048144.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1278&deploy=1&zee=' + debt_row.zeeName + '&start_date=' + startDate + '&last_date=' + lastDate + '" target=_blank>VIEW (Customer List)</a>';
                    ...
                    debtDataSet.push([detailedInvoiceURLMonth, month, debt_row.zeeName, debt_row.zeeNameText, debt_row.mpexUsage, debt_row.customerCount]);
                    csvSet.push([month, debt_row.zeeNameText, debt_row.mpexUsage, debt_row.customerCount]);
            ...
            for (var i = 0; i < data.length; i++) {
                zee_count.push(data[i][2]);
                mpex_usage[data[i][2]] = data[i][4];
                customer_count[data[i][2]] = data[i][5];
        ...
        function saveCsv(ordersDataSet) {
            var sep = "sep=;";
            var headers = ["Month", "Franchisee Name", "MPEX Usage", "Customer Count"]
