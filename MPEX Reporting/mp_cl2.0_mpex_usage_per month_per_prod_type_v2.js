    ...
    function(email, runtime, search, record, http, log, error, url, format, currentRecord) {
        ...
        function loadDebtRecord(date_from, date_to, zee_id) {
            ...
            mpexUsageResults.run().each(function(mpexUsageSet) {
                ...
                var productType = mpexUsageSet.getValue({
                    name: 'custrecord_cust_prod_stock_single_name',
                    summary: 'GROUP'
                });

                var productTypeText = mpexUsageSet.getText({
                    name: 'custrecord_cust_prod_stock_single_name',
                    summary: 'GROUP'
                });
        ...
        function loadDatatable(debt_rows) {
            ...
            if (!isNullorEmpty(debt_rows)) {
                debt_rows.forEach(function(debt_row, index) {
                    ...
                    var detailedInvoiceURLMonth = '<a href="https://1048144.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1271&deploy=1&zee=' + zee + '&start_date=' + startDate + '&last_date=' + lastDate + '" target=_blank>VIEW (per week)</a> | <a href="https://1048144.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1273&deploy=1&zee=' + zee + '&start_date=' + startDate + '&last_date=' + lastDate + '" target=_blank>VIEW (per day)</a>';



                    debtDataSet.push([detailedInvoiceURLMonth, month, debt_row.productTypeText, debt_row.mpexUsage, debt_row.customerCount, debt_row.zeeCount]);
                    csvSet.push([month, debt_row.productTypeText, debt_row.mpexUsage, debt_row.customerCount, debt_row.zeeCount]);
            ...
            var prod_type = []; //creating array for storing browser type in array.

            for (var i = 0; i < data.length; i++) {
                prod_type.push(data[i][2]);
                mpex_usage[data[i][2]] = data[i][3]
                customer_count[data[i][2]] = data[i][4]
                zee_count[data[i][2]] = data[i][5]
