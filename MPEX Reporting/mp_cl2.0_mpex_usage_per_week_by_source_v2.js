    ...
    function(email, runtime, search, record, http, log, error, url, format, currentRecord) {
        ...
        function loadDebtRecord(date_from, date_to, zee_id) {
            ...
            mpexUsageResults.run().each(function(mpexUsageSet) {
                ...
                console.log('dateUsed : ' + dateUsed);
                console.log('Source ID : ' + sourceId);
                console.log('Source Text : ' + sourceText);
                console.log('mpexUsage : ' + mpexUsage);
                ...
                if (old_date == null) {
                    ...
                    total_usage = source_manual + source_shopify + source_portal + source_bulk;
                ...
                console.log('source_manual : ' + source_manual);
                console.log('source_shopify : ' + source_shopify);
                console.log('source_portal : ' + source_portal);
                console.log('source_bulk : ' + source_bulk);
                console.log('total_usage : ' + total_usage);
        ...
        function loadDatatable(debt_rows) {
            ...
            if (!isNullorEmpty(debt_rows)) {
                debt_rows.forEach(function(debt_row, index) {
                    ...
                    var detailedInvoiceURLMonth = '<a href="https://1048144.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1283&deploy=1&zee=' + zee + '" target=_blank>VIEW (per month)</a> | <a href="https://1048144.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1287&deploy=1&zee=' + zee + '&start_date=' + startDate + '&last_date=' + lastDate + '" target=_blank>VIEW (per day)</a> | <a href="https://1048144.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1276&deploy=1&zee=' + zee + '&start_date=' + startDate + '&last_date=' + lastDate + '" target=_blank>VIEW (Product Types)</a> | <a href="https://1048144.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1278&deploy=1&zee=' + zee + '&start_date=' + startDate + '&last_date=' + lastDate + '" target=_blank>VIEW (Customer List)</a> | <a href="https://1048144.app.netsuite.com/app/site/hosting/scriptlet.nl?script=1280&deploy=1&zee=' + zee + '&start_date=' + startDate + '&last_date=' + lastDate + '" target=_blank>VIEW (Franchisee List)</a>';


                    debtDataSet.push([detailedInvoiceURLMonth, month, debt_row.source_manual, debt_row.source_portal, debt_row.source_shopify, debt_row.source_bulk, debt_row.total_usage]);
                    csvSet.push([month, debt_row.source_manual, debt_row.source_portal, debt_row.source_shopify, debt_row.source_bulk, debt_row.total_usage]);
            ...
            var sourceManual = []; //creating array for storing browser type in array.
            var sourcePortal = []; //creating array for storing browser type in array.
            var sourceBulk = []; //creating array for storing browser type in array.
            var sourceShopify = []; //creating array for storing browser type in array.
            var totalUsage = []; //creating array for storing browser type in array.

            for (var i = 0; i < data.length; i++) {
                month_year.push(data[i][1]);
                sourceManual[data[i][1]] = data[i][2]; //creating array for storing browser type in array.
                sourcePortal[data[i][1]] = data[i][3]; //creating array for storing browser type in array.
                sourceBulk[data[i][1]] = data[i][5]; //creating array for storing browser type in array.
                sourceShopify[data[i][1]] = data[i][4]; //
                totalUsage[data[i][1]] = data[i][6]; //
            ...
            var series_data5 = []; //creating empty array for highcharts series data
            var categores = []; //creating empty array for highcharts categories
            Object.keys(sourceManual).map(function(item, key) {
                console.log(item);
                series_data1.push(parseInt(sourceManual[item]));
                series_data2.push(parseInt(sourcePortal[item]));
                series_data3.push(parseInt(sourceBulk[item]));
                series_data4.push(parseInt(sourceShopify[item]));
                series_data5.push(parseInt(totalUsage[item]));
                categores.push(item)
            });
            plotChart(series_data1, series_data2, series_data3, series_data4, series_data5, categores)
            return true;
        }

        function plotChart(series_data1, series_data2, series_data3, series_data4, series_data5, categores) {
        ...
        function saveCsv(ordersDataSet) {
            var sep = "sep=;";
            // csvSet.push([month, debt_row.source_manual, debt_row.source_portal, debt_row.source_shopify, debt_row.source_bulk, debt_row.total_usage]);
            var headers = ["Month", "Manual", "Portal", "Shopify", "Bulk", "Total Usage"]
