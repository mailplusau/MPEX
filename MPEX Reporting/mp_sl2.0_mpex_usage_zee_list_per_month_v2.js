...
define(['N/ui/serverWidget', 'N/email', 'N/runtime', 'N/search', 'N/record', 'N/http', 'N/log', 'N/redirect'],
	function(ui, email, runtime, search, record, http, log, redirect) {
			...
			if (context.request.method === 'GET') {
				...
				if (isNullorEmpty(context.request.parameters.zee)) {
					var form = ui.createForm({
						title: 'MPEX Monthly Usage - Franchisee List'
					});
				} else {
					...
					var form = ui.createForm({
						title: zee_name + 'MPEX Monthly Usage - Franchisee List'
					});
				}
				...
				inlineHtml += sourceDropdownSection(source, context);
				...
				form.clientScriptFileId = 4999034;
         ...
         */
        function sourceDropdownSection(source, context) {
            var source_text = ['Manual', 'Shopify', 'Portal', 'Bulk'];


            var inlineHtml = '<div class="form-group container source_section">';
            inlineHtml += '<div class="row">';
            inlineHtml += '<div class="col-xs-12 heading1"><h4><span class="label label-default col-xs-12" style="background-color: #103D39;">SOURCE</span></h4></div>';
            inlineHtml += '</div>';
            inlineHtml += '</div>';
            ...
            // 3 - Customer Portal
            // 4 - Bulk
            if (context.request.parameters.source == source) {
                inlineHtml += '<option value="' + source + '" selected="selected">' + source_text[source - 1] + '</option>';
            } else {
