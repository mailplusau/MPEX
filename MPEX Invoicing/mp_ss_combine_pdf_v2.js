 ...
 * @Last modified time: 2021-11-01T09:09:47+11:00
 *
 */

var prod_usage_report = [196, 197, 198, 199, 200, 201, 202, 203, 204, 205, 206,
  207, 208, 209, 210, 211, 212, 213, 214, 215, 226, 227, 228, 229, 230, 231,
  232, 233, 234, 235, 243, 244, 245
];
...
function main() {
  ...
  var customerList = nlapiLoadSearch('customrecord_mp_ap_product_order',
    'customsearch_mpex_product_invoice_list_4');
  if (!isNullorEmpty(customerID)) {
    var newFilters = new Array();
    newFilters[newFilters.length] = new nlobjSearchFilter(
      'custrecord_ap_order_customer', null, 'anyof', customerID);
  ...
  resultCustomerList.forEachResult(function(searchResultCustomer) {
    var cust_prod_customer = searchResultCustomer.getValue(
    ...
    var createProdOrderSearch = nlapiLoadSearch(
      ...
      'customsearch_mpex_product_invoice_list_3');
    // if (!isNullorEmpty(customerID)) {
    var newFilters = new Array();
    newFilters[newFilters.length] = new nlobjSearchFilter(
      'custrecord_ap_order_customer', null, 'anyof', cust_prod_customer);
    createProdOrderSearch.addFilters(newFilters);
    // }
    ...
    resultCreateProdOrder.forEachResult(function(searchResult) {
      var cust_prod_stock_id = searchResult.getValue("internalid", null,
        "GROUP");
      var cust_prod_item = searchResult.getText(
        "custrecord_ap_stock_line_item",
        "CUSTRECORD_AP_PRODUCT_ORDER", "GROUP");
      var cust_prod_order_inv = searchResult.getValue("tranid",
        "CUSTRECORD_MP_AP_ORDER_INVOICENUM", "GROUP");
      var inv_id = searchResult.getValue("internalid",
        "CUSTRECORD_MP_AP_ORDER_INVOICENUM", "GROUP");
      var cust_prod_customer = searchResult.getValue(
        "custrecord_ap_order_customer", null, "GROUP");

      var line_item_details = searchResult.getValue(
      ...
      if (cust_prod_customer != old_customer_id && !isNullorEmpty(
          old_customer_id)) {
        ...
        if (!isNullorEmpty(old_customer_id)) {
          ...
          for (p = 1; p <= recCustomer.getLineItemCount('addressbook'); p++) {
            if (recCustomer.getLineItemValue('addressbook',
                'defaultbilling', p) == "T") {
              if (!isNullorEmpty(recCustomer.getLineItemValue(
                  'addressbook', 'addr1', p))) {
                billaddressfull += recCustomer.getLineItemValue(
                  'addressbook', 'addr1', p) + ',';
              }
              if (!isNullorEmpty(recCustomer.getLineItemValue(
                  'addressbook', 'addr2', p))) {
          ...
          for (x = 0; x < pages; x++) {
            ...
            fileSCFORM.setName('MPEX_ProductUsageReport_' + getDate() +
          ...
          if (!isNullorEmpty(all_pages)) {
            ...
            file.setName('MPEX_ProductUsageReport_all_' + getDate() +
              '_' + old_customer_id + '.pdf');
          ...
          inv_record.setFieldValue('custbody_mpex_usage_report',
            all_pages_id);
          ...
          if (old_pord_array.length > 0) {
            for (var i = 0; i < old_pord_array.length; i++) {
              var product_order_rec = nlapiLoadRecord(
                'customrecord_mp_ap_product_order', old_pord_array[i]
              );
              product_order_rec.setFieldValue(
                'custrecord_prod_usage_report', all_pages_id);
              product_order_rec.setFieldValue('custrecord_usage_report',
                1);
              product_order_id = nlapiSubmitRecord(product_order_rec);
            }
          } else {
            var product_order_rec = nlapiLoadRecord(
              'customrecord_mp_ap_product_order', old_prod_id);
            product_order_rec.setFieldValue(
              'custrecord_prod_usage_report', all_pages_id);
            product_order_rec.setFieldValue('custrecord_usage_report',
              1);
        ...
        if (usageStart <= usageThreshold) {
          ...
          nlapiLogExecution('EMERGENCY', 'Parameters Passed | ',
            'Customer ID: ' + old_customer_id);

          var reschedule = rescheduleScript(prevInvDeploy,
            adhocInvDeploy, params);
          if (reschedule == false) {
            return false;
          }
        }
      ...
      if (old_prod_id != cust_prod_stock_id && !isNullorEmpty(
          old_prod_id)) {
    ...
    if (!isNullorEmpty(old_customer_id)) {
      ...
      var customerName = recCustomer.getFieldValue('companyname');
      for (p = 1; p <= recCustomer.getLineItemCount('addressbook'); p++) {
        if (recCustomer.getLineItemValue('addressbook', 'defaultbilling', p) ==
          "T") {
      ...
      for (x = 0; x < pages; x++) {
        ...
        var fileSCFORM = nlapiMergeRecord(prod_usage_report[x], 'customer',
          old_customer_id, null, null, merge);
        fileSCFORM.setName('MPEX_ProductUsageReport_' + getDate() + '_' +
          old_customer_id + '_' + (x + 1) + '.pdf');
        fileSCFORM.setIsOnline(true);
        fileSCFORM.setFolder(2177205);
      ...
      if (old_pord_array.length > 0) {
        for (var i = 0; i < old_pord_array.length; i++) {
          var product_order_rec = nlapiLoadRecord(
            'customrecord_mp_ap_product_order', old_pord_array[i]);
          product_order_rec.setFieldValue('custrecord_prod_usage_report',
            all_pages_id);
