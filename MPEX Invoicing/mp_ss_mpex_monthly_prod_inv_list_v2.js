...
function rescheduleScript(script_deployment_1, script_deployment_2, params) {
	...
	if (current_script_deployment == script_deployment_1) {
		...
		throw nlapiCreateError(
			...
			current_script_deployment + "not defined in rescheduleScript",
			false
		);
		return false;
	}
...
function main(type) {
	...
	if (!isNullorEmpty(searchResults)) {
		...
		for (var n = 0; n < searchResults.length; n++) {
			...
			if (
				ctx.getRemainingUsage() <= usage_threshold &&
				n + 1 < searchResults.length
			) {
				var params = {
			...
			var customer_po = searchResults[n].getValue(
				"custentity11",
				"CUSTRECORD_AP_ORDER_CUSTOMER",
				"GROUP"
			);
			var mpex_po = searchResults[n].getValue(
				"custentity_mpex_po",
				"CUSTRECORD_AP_ORDER_CUSTOMER",
				"GROUP"
			);
			var product_po = searchResults[n].getValue(
				"custrecord_mp_ap_order_po",
				"GROUP"
			);
			var ordered_by = searchResults[n].getValue(
				"custrecord_mp_ap_order_ordered_by",
				"GROUP"
			);
			...
			var rasTier1Count = searchResults[n].getValue(
				"custrecord_total_ras_teir1_barcode_count",
				null,
				"GROUP"
			);
			var rasTier2Count = searchResults[n].getValue(
				"custrecord_total_ras_teir2_barcode_count",
				null,
				"GROUP"
			);
			var rasTier3Count = searchResults[n].getValue(
				"custrecord_total_ras_teir3_barcode_count",
				null,
				"GROUP"
			);
			var manualBarcodeCount = searchResults[n].getValue(
				"custrecord_total_manual_barcode_count",
				null,
				"GROUP"
			);
			...
			nlapiLogExecution(
				"DEBUG",
				"Product Internal ID",
				searchResults[n].getValue("internalid", null, "GROUP")
			);
			...
			try {
				if (
					searchResults[n].getValue("custrecord_mp_ap_order_from_swap") != "T"
				) {
					if (
						customer_ids ==
						...
						new_invoice = false;
						if (
							(manual_surcharge == 1 || manual_surcharge == "1") &&
						...
						internal_id[internal_id.length] = parseInt(
							searchResults[n].getValue("internalid", null, "GROUP")
						);
					} else {
						new_invoice = true;
					...
					if (n == 0) {
						...
						recInvoice.setFieldValue(
							"department",
							nlapiLoadRecord("partner", 435).getFieldValue("department")
						);
						recInvoice.setFieldValue(
							"location",
						...
						recInvoice.setFieldValue("trandate", "31/03/2025");
						recInvoice.setFieldValue(
						...
						if (
							!isNullorEmpty(mpex_po) ||
							(isNullorEmpty(product_po) && isNullorEmpty(customer_po))
						) {
					...
					if (new_invoice == true && n > 0) {
						...
						var fil_po = [];
						fil_po[fil_po.length] = new nlobjSearchFilter(
							"internalid",
							null,
							"anyof",
							old_line_items
						...
						if (
							...
							//IF PRICING ALGORITHM: PRICE LISTS
							...
							for (var x = 0; x < item_rates.length; x++) {
								...
								var apItemDeliverySpeed = poSearch[0].getValue(
									"custrecord_ap_item_delivery_speed"
								);
								...
								if (y != "") {
									if (parseInt(line_qty) < y) {
										...
										if (
											!isNullorEmpty(inv_details) ||
											!isNullorEmpty(ordered_by)
										) {
											var inv_details_rec = nlapiCreateRecord("customrecord62");
											if (!isNullorEmpty(ordered_by)) {
												var new_inv_details =
													"Order By - " + ordered_by + ". " + inv_details;
											} else {
										...
										recInvoice.setFieldValue(
											"custbody_mp_std_included",
											mpStdIncluded
										);
										recInvoice.setFieldValue(
											"custbody_mp_exp_included",
											mpExpIncluded
										);
										recInvoice.setFieldValue(
											"custbody_mp_prm_included",
											mpPrmIncluded
										);
										break;
									}
						...
						nlapiLogExecution(
							"AUDIT",
							"Loop: " + invCount + "." + n + " | Product IDS: " + internal_id,
							usage_per_loop - ctx.getRemainingUsage()
						);
						...
						recInvoice.setFieldValues(
							"custbody_mpex_product_order",
							internal_id
						);
						...
						if (!isNullorEmpty(invoiceId)) {
							for (var z = 0; z < internal_id.length; z++) {
								...
								recOrder.setFieldValue(
									"custrecord_mp_ap_order_order_status",
									6
								);
								recOrder.setFieldValue(
									"custrecord_mp_ap_order_invoicenum",
									invoiceId
								);
							...
							if (
								...
								for (var x = 0; x < item_rates.length; x++) {
									...
									if (y != "") {
										...
										var item_selected =
											"custrecord_ap_item_" + item_rates[x - 1];
		...
		//--------------- Apply AP Item Pricing Algorithm ---------------//
		if (
			poSearch[0].getValue("custrecord_ap_item_pricing_algorithm") == 1 ||
			isNullorEmpty(
				poSearch[0].getValue("custrecord_ap_item_pricing_algorithm")
			)
		) {
			...
			message +=
				" | OrderID:" + searchResults[n].getValue("internalid", null, "GROUP");
		...
		if (parseInt(oldRASTier1Count) > 0) {
			recInvoice.selectNewLineItem("item");
			recInvoice.setCurrentLineItemValue("item", "item", 10782);
			recInvoice.setCurrentLineItemValue("item", "quantity", oldRASTier1Count);
			recInvoice.commitLineItem("item");
		}
		...
		if (parseInt(oldRASTier2Count) > 0) {
			recInvoice.selectNewLineItem("item");
			recInvoice.setCurrentLineItemValue("item", "item", 10783);
			recInvoice.setCurrentLineItemValue("item", "quantity", oldRASTier2Count);
			recInvoice.commitLineItem("item");
		}
		...
		if (parseInt(oldRASTier3Count) > 0) {
			recInvoice.selectNewLineItem("item");
			recInvoice.setCurrentLineItemValue("item", "item", 10784);
			recInvoice.setCurrentLineItemValue("item", "quantity", oldRASTier3Count);
			recInvoice.commitLineItem("item");
		}
