...
function getLatestFiles() {
  if (
    !isNullorEmpty(ctx.getSetting("SCRIPT", "custscript_prev_deploy_scan_json"))
  ) {
    prev_inv_deploy = ctx.getSetting(
      "SCRIPT",
  ...
  resultScanJSONSearch.forEachResult(function (searchResultScanJSON) {
    ...
    var scan_json_record = nlapiLoadRecord(
      "customrecord_scan_json",
      scan_json_record_id
    );
    ...
    if (barcodes.length > 0) {
      ...
      do {
        ...
        var sendle_service_product = barcodes[x].sendle_service_product;
        var starTrack_api_price = barcodes[x].order_total_startrack_price;
        var starTrack_api_total_cost_ex_gst =
          barcodes[x].package_startrack_total_cost_ex_gst;
        var starTrack_api_shipping_cost =
          barcodes[x].package_startrack_shipping_cost;
        var starTrack_api_fuel_surcharge =
          barcodes[x].package_startrack_fuel_surcharge;
        var starTrack_api_total_gst = barcodes[x].package_startrack_total_gst;
        var starTrack_freight_charge =
          barcodes[x].package_startrack_freight_charge;
        ...
        if ("order_total_price" in barcodes[x]) {
          nlapiLogExecution(
            "DEBUG",
            "order_total_price exists",
            barcodes[x].order_total_price
          );
        ...
        if ("package_price" in barcodes[x]) {
          nlapiLogExecution(
            "DEBUG",
            "package_price exists",
            barcodes[x].package_price
          );
        ...
        if ("payment" in barcodes[x]) {
          ...
          if (!isNullorEmpty(barcodes[x].payment)) {
            ...
            ccPaymentDate = nlapiStringToDate(
              ccPaymentDate[2] + "/" + ccPaymentDate[1] + "/" + ccPaymentDate[0]
            );
        ...
        for (var y = 0; y < scans.length; y++) {
          ...
          if (!isNullorEmpty(customer_id)) {
            ...
            var customer1stBarcodeUsed = customerRecord.getFieldValue(
              "custentity_mp_prod_1st_barcode_used"
            );
            ...
            if (
              isNullorEmpty(customer1stBarcodeUsed) &&
              customerStatus == 73 &&
              scans.length - 1 == y &&
              (scan_type == "pickup" ||
                scan_type == "delivery" ||
          ...
          if (
            ...
            !isNullorEmpty(zeeIDCustomerRecord)
          ) {
            zee_id = zeeIDCustomerRecord;
          }
          ...
          if (
            !isNullorEmpty(receiver_suburb) &&
            !isNullorEmpty(receiver_postcode)
          ) {
          ...
          if (!isNullorEmpty(order_date)) {
            ...
            order_date = nlapiStringToDate(
              order_date[2] + "/" + order_date[1] + "/" + order_date[0]
            );
          }

          if (
            !isNullorEmpty(eta_delivery_date_min) &&
            !isNullorEmpty(eta_delivery_date_max)
          ) {
          ...
          updated_at = nlapiStringToDate(
            updated_at[2] + "/" + updated_at[1] + "/" + updated_at[0]
          );
          ...
          resultSetProductStock.forEachResult(function (searchResult) {
            ...
            customer_prod_stock.setFieldValue("custrecord_height", height);
            customer_prod_stock.setFieldValue("custrecord_weight", weight);
            customer_prod_stock.setFieldValue("custrecord_length", length);
            customer_prod_stock.setFieldValue("custrecord_width", width);
            customer_prod_stock.setFieldValue(
              "custrecord_ext_reference_id",
              reference_id
            );
            customer_prod_stock.setFieldValue("custrecord_job_id", job_id);
            customer_prod_stock.setFieldValue(
              "custrecord_order_date",
              order_date
            );
            customer_prod_stock.setFieldValue(
              "custrecord_item_price",
              item_price
            );
            customer_prod_stock.setFieldValue(
              "custrecord_order_number",
              order_number
            );
            customer_prod_stock.setFieldValue(
              "custrecord_order_total_price",
              order_total_price
            );
            customer_prod_stock.setFieldValue(
            ...
            customer_prod_stock.setFieldValue(
              ...
              starTrack_api_total_cost_ex_gst
            );
            customer_prod_stock.setFieldValue(
              "custrecord_st_shipping_cost",
              starTrack_api_shipping_cost
            );
            customer_prod_stock.setFieldValue(
              "custrecord_st_fuel_surcharge",
              starTrack_api_fuel_surcharge
            );
            customer_prod_stock.setFieldValue(
              "custrecord_st_total_gst",
              starTrack_api_total_gst
            );
            customer_prod_stock.setFieldValue(
              "custrecord_st_freight_charge",
              starTrack_freight_charge
            );
            customer_prod_stock.setFieldValue(
              "custrecord_st_security_surcharge",
              starTrack_security_surcharge
            );
            ...
            if (currentBarcodeRASTier1 == true) {
              customer_prod_stock.setFieldValue("custrecord_tge_ras", "Tier 1");
            } else if (currentBarcodeRASTier2 == true) {
              customer_prod_stock.setFieldValue("custrecord_tge_ras", "Tier 3");
            ...
            if (!isNullorEmpty(transactionID)) {
              customer_prod_stock.setFieldValue(
                "custrecord_cust_prod_stock_invoiceable",
                2
              );
              customer_prod_stock.setFieldValue(
                "custrecord_credit_card_payment",
                1
              );
              customer_prod_stock.setFieldValue(
                "custrecord_cc_transaction_payment_price",
                paymentTotalAmount
              );
              customer_prod_stock.setFieldValue(
                "custrecord_cc_payment_surcharge",
                paymentSurcharge
              );
              customer_prod_stock.setFieldValue(
                "custrecord_cc_courier_surcharge",
                courierSurcharge
              );
              customer_prod_stock.setFieldValue(
                "custrecord_credit_card_type",
                cardType
              );
              customer_prod_stock.setFieldValue(
                "custrecord_cc_payment_date",
                ccPaymentDate
              );
              customer_prod_stock.setFieldValue(
                "custrecord_cc_payment_time",
                timeCCPayment
              );
              customer_prod_stock.setFieldValue(
            ...
            if (stock_status != 6 && stock_status != 7) {
              if (!isNullorEmpty(deleted)) {
                if (account == "sendle" && product_type == null) {
                  if (stock_status == 2) {
                    //Existing Status is Pickup
                  } else if (stock_status == 4 || stock_status == 5) {
                    //Existing Status is Lodged or Delivered
                  }
                } else {
                  ...
                  } else if (stock_status == 8) {
                    // Status is Zee
                    // Stock
                    customer_prod_stock.setFieldValue("isinactive", "T");
                    customer_prod_stock.setFieldValue(
                      "custrecord_cust_time_deleted",
                      time_updated_at
                    );
                  } else if (stock_status == 4 || stock_status == 5) {
                    // Status
                ...
                if (!isNullorEmpty(customer_id)) {
                  var searchProductPricing = nlapiLoadSearch(
                  ...
                  if (
                    ...
                    var newFilterExpression = [
                      ...
                      "AND",
                      ["custrecord_prod_pricing_delivery_speeds", "anyof", 2],
                      "AND",
                      ["custrecord_prod_pricing_carrier_last_mil", "anyof", 2],
                      "AND",
                    ...
                    var newFilterExpression = [
                      ...
                      "AND",
                      ["custrecord_prod_pricing_delivery_speeds", "anyof", 1],
                      "AND",
                      ["custrecord_prod_pricing_status", "anyof", [2, 6]]
                    ];
                  } else if (delivery_speed == "Premium Express") {
                    var newFilterExpression = [
                      ...
                      "AND",
                      ["custrecord_prod_pricing_delivery_speeds", "anyof", 4],
                      "AND",
                      ["custrecord_prod_pricing_status", "anyof", [2, 6]]
                    ];
                  }
                  ...
                  var resultSetProductPricing =
                    searchProductPricing.runSearch();

                  var firstResult = resultSetProductPricing.getResults(0, 1);

                  if (firstResult.length > 0) {
                    var prodPricingInternalID =
                      firstResult[0].getValue("internalid");
                    ...
                    if (!isNullorEmpty(prodItemText)) {
                      var searchAPItems = nlapiLoadSearch(
                        "customrecord_ap_item",
                        "customsearch6413"
                      );
                      ...
                      if (delivery_speed == "Standard") {
                        if (!isNullorEmpty(delivery_zone)) {
                          if (delivery_zone.toUpperCase() == "REMOTE") {
                            prodItemText =
                              prodItemText.slice(0, -1) + ", D:REM)";
                          } else if (
                            delivery_zone.toUpperCase() == "REMOTE_WANT"
                          ) {
                            prodItemText =
                        ...
                        if (!isNullorEmpty(delivery_zone)) {
                          if (delivery_zone.toUpperCase() == "REMOTE") {
                            ...
                            prodItemText =
                              removeTrailingWhitespace(prodItemText) + " ";
                            prodItemText =
                              prodItemText.slice(0, -1) + " (D: REM)";
                          }
                        }
                      ...
                      var newFilterExpressionAPItem = [
                        [
                          "custrecord_ap_item_default.custitem_price_plans",
                          "anyof",
                          "13",
                          "14",
                          "15",
                        ...
                        ["isinactive", "is", "F"],
                        "AND",
                        ["name", "is", prodItemText]
                      ];

                      searchAPItems.setFilterExpression(
                        newFilterExpressionAPItem
                      );
                      ...
                      nlapiLogExecution(
                        "DEBUG",
                        "firstResultAPItem.length 1",
                        firstResultAPItem.length
                      );
                      if (firstResultAPItem.length > 0) {
                        var apItemInternalID =
                          firstResultAPItem[0].getValue("internalid");
                      }
                      nlapiLogExecution(
                        "DEBUG",
                        "apItemInternalID",
                  ...
                  if (
                    ...
                    customer_prod_stock.setFieldValue(
                      "custrecord_cust_prod_stock_prepaid",
                      1
                    );
                  }
                  ...
                  customer_prod_stock.setFieldValue(
                    "custrecord_cust_date_stock_given",
                    updated_at
                  );
                  customer_prod_stock.setFieldValue(
                  ...
                  customer_prod_stock.setFieldValue(
                    "custrecord_history_pickup_date",
                    updated_at
                  );
                  customer_prod_stock.setFieldValue(
                    "custrecord_history_pickup_time",
                    time_updated_at
                  );
                } else {
                  nlapiSendEmail(
                    ...
                    "MPEX Scan Sync",
                    "Barcode: " + barcode + " has empty Customer ID",
                    null
                  );
                  save_barcode = false;
                }
                ...
                if (!isNullorEmpty(zee_id)) {
                  ...
                  customer_prod_stock.setFieldValue(
                    "custrecord_history_zee_stock_date",
                    updated_at
                  );
                  customer_prod_stock.setFieldValue(
                  ...
                  nlapiSendEmail(
                    ...
                    ["ankith.ravindran@mailplus.com.au"],
                    "MPEX Scan Sync",
                    "Barcode: " + barcode + " has empty Zee ID",
                    null
                  );
                  save_barcode = false;
                }
                ...
                if (!isNullorEmpty(customer_id)) {
                  ...
                  customer_prod_stock.setFieldValue(
                    "custrecord_history_allocate_date",
                    updated_at
                  );
                  customer_prod_stock.setFieldValue(
                ...
                if (!isNullorEmpty(customer_id)) {
                  ...
                  if (
                    ...
                    var newFilterExpression = [
                      ...
                      ["custrecord_prod_pricing_delivery_speeds", "anyof", 2],
                      "AND",
                      ["custrecord_prod_pricing_carrier_last_mil", "anyof", 1],
                      "AND",
                      ["custrecord_prod_pricing_status", "anyof", [2, 6]]
                    ];
                  } else if (delivery_speed == "Standard") {
                  ...
                  customer_prod_stock.setFieldValue(
                    "custrecord_history_delivery_date",
                    updated_at
                  );
                  customer_prod_stock.setFieldValue(
                ...
                if (!isNullorEmpty(customer_id)) {
                  ...
                  customer_prod_stock.setFieldValue(
                    "custrecord_history_lodge_date",
                    updated_at
                  );
                  customer_prod_stock.setFieldValue(
                    "custrecord_history_lodge_time",
                    time_updated_at
                  );
                } else {
                  nlapiSendEmail(
              ...
              if (save_barcode == true) {
                customer_prod_stock.setFieldValue(
                  "custrecord_cust_prod_stock_source",
                  6
                );
                customer_prod_stock.setFieldValue(
                  "custrecord_connote_number",
                  connote_number
                );
                customer_prod_stock.setFieldValue(
                  "custrecord_cust_prod_stock_operator",
                  operator_id
                );
                if (
                  invoiceable === false ||
                ...
                customer_prod_stock.setFieldValue(
                  "custrecord_mpdl_number",
                  external_barcode
                );
                customer_prod_stock.setFieldValue(
                  "custrecord_barcode_source",
                  sourceId
                );
                if (!isNullorEmpty(futile_reasons)) {
                  customer_prod_stock.setFieldValue(
                    "custrecord_futile_reasons",
                    futile_reasons
                  );
                }
                customer_prod_stock.setFieldValue(
                ...
                customer_prod_stock.setFieldValue(
                  "custrecord_receiver_email",
                  receiver_email
                );
                customer_prod_stock.setFieldValue(
                  "custrecord_receiver_phone",
                  receiver_phone
                );
                ...
                customer_prod_stock.setFieldValue(
                  "custrecord_senders_name",
                  sender_name
                );
                customer_prod_stock.setFieldValue(
                  "custrecord_sender_email",
                  sender_email
                );
                customer_prod_stock.setFieldValue(
                ...
                customer_prod_stock.setFieldValue(
                  "custrecord_sender_address_1",
                  sender_address1
                );
                customer_prod_stock.setFieldValue(
                  "custrecord_sender_address_2",
                  sender_address2
                );
                customer_prod_stock.setFieldValue(
                  "custrecord_senders_phone",
                  sender_phone
                );
                ...
                if (account == "sendle") {
                  if (product_type == null) {
                    ...
                    if (sendle_service_product == "B2C Priority ATL") {
                      customer_prod_stock.setFieldValue(
                        "custrecord_delivery_speed",
                        5
                      );
                    } else if (sendle_service_product == "B2C Std ATL") {
                      customer_prod_stock.setFieldValue(
                        "custrecord_delivery_speed",
                        6
                      );
                    ...
                    customer_prod_stock.setFieldValue(
                      "custrecord_eta_del_date_min",
                      eta_delivery_date_min
                    );
                    customer_prod_stock.setFieldValue(
                      "custrecord_eta_del_date_max",
                      eta_delivery_date_max
                    );
                    if (!isNullorEmpty(delivery_zone)) {
                      if (delivery_zone.toUpperCase() == "NATIONAL") {
                        customer_prod_stock.setFieldValue(
                          "custrecord_delivery_zone",
                          1
                        );
