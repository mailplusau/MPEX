 ...
 * @Last modified time: 2021-11-01T08:54:24+11:00
 *
 */
...
function pageInit() {
  ...
  var prodStockSearch = nlapiLoadSearch('customrecord_customer_product_stock',
    'customsearch_prod_stock_refill_custome_2');

  var addFilterExpression = new nlobjSearchFilter('partner',
    'CUSTRECORD_CUST_PROD_STOCK_CUSTOMER', 'anyof', parseInt(
      nlapiGetFieldValue('zee')));
  ...
  resultSetCustomer.forEachResult(function(searchResult) {
    ...
    var custid = searchResult.getValue(
      "custrecord_cust_prod_stock_customer", null, "GROUP");
    var zee_id = searchResult.getValue("partner",
      "CUSTRECORD_CUST_PROD_STOCK_CUSTOMER", "GROUP");
    var companyname = searchResult.getText(
      "custrecord_cust_prod_stock_customer", null, "GROUP");
    var companyname_text = searchResult.getValue("companyname",
      "CUSTRECORD_CUST_PROD_STOCK_CUSTOMER", "GROUP");

    var mpen_total_stock = searchResult.getValue("custentity_mpen",
      "CUSTRECORD_CUST_PROD_STOCK_CUSTOMER", "GROUP");
    if (isNullorEmpty(mpen_total_stock)) {
      mpen_total_stock = 0;
    }
    var mpen_min_float = searchResult.getValue("custentity_mpex_1kg_float",
      "CUSTRECORD_CUST_PROD_STOCK_CUSTOMER", "GROUP");
    if (isNullorEmpty(mpen_min_float)) {
      mpen_min_float = 0;
    }

    var mpet_total_stock = searchResult.getValue("custentity_mpet",
      "CUSTRECORD_CUST_PROD_STOCK_CUSTOMER", "GROUP");
    if (isNullorEmpty(mpet_total_stock)) {
      mpet_total_stock = 0;
    }
    var mpet_min_float = searchResult.getValue("custentity_mpex_3kg_float",
      "CUSTRECORD_CUST_PROD_STOCK_CUSTOMER", "GROUP");
    if (isNullorEmpty(mpet_min_float)) {
      mpet_min_float = 0;
    }

    var mpef_total_stock = searchResult.getValue("custentity_mpef",
      "CUSTRECORD_CUST_PROD_STOCK_CUSTOMER", "GROUP");
    if (isNullorEmpty(mpef_total_stock)) {
      mpef_total_stock = 0;
    }
    var mpef_min_float = searchResult.getValue("custentity_mpex_5kg_float",
      "CUSTRECORD_CUST_PROD_STOCK_CUSTOMER", "GROUP");
    if (isNullorEmpty(mpef_min_float)) {
      mpef_min_float = 0;
    }

    var mpeb_total_stock = searchResult.getValue("custentity_mpeb",
      "CUSTRECORD_CUST_PROD_STOCK_CUSTOMER", "GROUP");
    if (isNullorEmpty(mpeb_total_stock)) {
      mpeb_total_stock = 0;
    }
    var mpeb_min_float = searchResult.getValue("custentity_mpex_b4_float",
      "CUSTRECORD_CUST_PROD_STOCK_CUSTOMER", "GROUP");
    if (isNullorEmpty(mpeb_min_float)) {
      mpeb_min_float = 0;
    }

    var mpec_total_stock = searchResult.getValue("custentity_mpec",
      "CUSTRECORD_CUST_PROD_STOCK_CUSTOMER", "GROUP");
    if (isNullorEmpty(mpec_total_stock)) {
      mpec_total_stock = 0;
    }
    var mpec_min_float = searchResult.getValue("custentity_mpex_c5_float",
      "CUSTRECORD_CUST_PROD_STOCK_CUSTOMER", "GROUP");
    if (isNullorEmpty(mpec_min_float)) {
      mpec_min_float = 0;
    }

    var mped_total_stock = searchResult.getValue("custentity_mped",
      "CUSTRECORD_CUST_PROD_STOCK_CUSTOMER", "GROUP");
    if (isNullorEmpty(mped_total_stock)) {
      mped_total_stock = 0;
    }
    var mped_min_float = searchResult.getValue("custentity_mpex_dl_float",
      "CUSTRECORD_CUST_PROD_STOCK_CUSTOMER", "GROUP");
    if (isNullorEmpty(mped_min_float)) {
      mped_min_float = 0;
    }

    var mpeg_total_stock = searchResult.getValue("custentity_mpeg",
      "CUSTRECORD_CUST_PROD_STOCK_CUSTOMER", "GROUP");
    if (isNullorEmpty(mpeg_total_stock)) {
      mpeg_total_stock = 0;
    }
    var mpeg_min_float = searchResult.getValue("custentity_mpex_500g_float",
      "CUSTRECORD_CUST_PROD_STOCK_CUSTOMER", "GROUP");
    ...
    dataSet += '{"cust_id":"' + custid + '","companyname_text":"' +
      companyname_text + '", "company_name":"' + companyname +
      '","mpen_stock": "' + mpen_total_stock + '","mpen_min": "' +
      mpen_min_float + '","mpet_stock": "' + mpet_total_stock +
      '","mpet_min": "' + mpet_min_float + '","mpef_stock": "' +
      mpef_total_stock + '","mpef_min": "' + mpef_min_float +
      '","mpeb_stock": "' + mpeb_total_stock + '","mpeb_min": "' +
      mpeb_min_float + '","mpec_stock": "' + mpec_total_stock +
      '","mpec_min": "' + mpec_min_float + '","mped_stock": "' +
      mped_total_stock + '","mped_min": "' + mped_min_float +
      '","mpeg_stock": "' + mpeg_total_stock + '","mpeg_min": "' +
      mpeg_min_float + '"},';
    ...
    table = $("#customer").DataTable({
      ...
      "columns": [
        ...
        // },
        {
          ...
          "render": function(data, type, row) {
            return
              '<input type="number" class="form-control 5kg_min text-center " value="' +
              data.mpef_min + '" old_value="' + data.mpef_min +
              '">';
          }
        }, {
          "data": null,
          "render": function(data, type, row) {
            if (parseInt(data.mpef_min) > parseInt(data.mpef_stock)) {
              return
                '<input type="number" class="form-control text-center has-error" style="color:red; border: solid !important;" value="' +
                data.mpef_stock + '" readonly>';
            } else if (parseInt(data.mpef_min) == parseInt(data.mpef_stock)) {
              return
                '<input type="number" class="form-control text-center has-warning" style="color:orange; border: solid !important;" value="' +
                data.mpef_stock + '" readonly>';
            } else {
              return
                '<input type="number" class="form-control text-center" style="color:green;" value="' +
                data.mpef_stock + '" readonly>';
            }
          ...
          "render": function(data, type, row) {
            return
              '<input type="number" class="form-control 3kg_min text-center" value="' +
              data.mpet_min + '" old_value="' + data.mpet_min +
              '">';
          }
        }, {
          "data": null,
          ...
          "render": function(data, type, row) {
            return
              '<input type="number" class="form-control 1kg_min text-center" value="' +
              data.mpen_min + '" old_value="' + data.mpen_min +
              '">';
          }
        }, {
          "data": null,
          "render": function(data, type, row) {
            if (parseInt(data.mpen_min) > parseInt(data.mpen_stock)) {
              return
                '<input type="number" class="form-control text-center has-error" style="color:red; border: solid !important;" value="' +
                data.mpen_stock + '" readonly>';
            } else if (parseInt(data.mpen_min) == parseInt(data.mpen_stock) &&
              parseInt(data.mpen_min) != 0 && parseInt(data.mpen_stock) !=
              0) {
              return
                '<input type="number" class="form-control text-center has-warning" style="color:orange; border: solid !important;" value="' +
                data.mpen_stock + '" readonly>';
          ...
          "render": function(data, type, row) {
            return
              '<input type="number" class="form-control 500g_min text-center " value="' +
              data.mpeg_min + '" old_value="' + data.mpeg_min +
              '">';
          }
        }, {
          "data": null,
          ...
          "render": function(data, type, row) {
            return
              '<input type="number" class="form-control b4_min text-center" value="' +
              data.mpeb_min + '" old_value="' + data.mpeb_min +
              '">';
          }
        }, {
          "data": null,
          ...
          "render": function(data, type, row) {
            return
              '<input type="number" class="form-control c5_min text-center" value="' +
              data.mpec_min + '" old_value="' + data.mpec_min +
              '">';
          }
        }, {
          "data": null,
          ...
          "render": function(data, type, row) {
            return
              '<input type="number" class="form-control dl_min text-center" value="' +
              data.mped_min + '" old_value="' + data.mped_min +
              '">';
          }
        }, {
          "data": null,
...
function onclick_back() {
  ...
  var upload_url = baseURL + nlapiResolveURL('SUITELET',
      'customscript_sl_full_calendar', 'customdeploy_sl_full_calender') +
    '&unlayered=T&zee=' + parseInt(nlapiGetFieldValue('zee')) +
...
function saveRecord() {
  ...
  for (var x = 0; x < customer_id_elem.length; x++) {
    ...
    if (mpen_elem[x].value != mpen_elem[x].getAttribute('old_value') ||
      mpet_elem[x].value != mpet_elem[x].getAttribute('old_value') || mpef_elem[
        x].value != mpef_elem[x].getAttribute('old_value') || mpeb_elem[x].value !=
      mpeb_elem[x].getAttribute('old_value') || mpec_elem[x].value != mpec_elem[
        x].getAttribute('old_value') || mped_elem[x].value != mped_elem[x].getAttribute(
        'old_value') || mpeg_elem[x].value != mpeg_elem[x].getAttribute(
        'old_value')) {
